...
public class InproceedingsReference extends AbstractReference {
    ...
    public InproceedingsReference(String name, Map<EntryType, String> entries) {
        super(name, entries, INPROCEEDINGS, new ArrayList<EntryType>(Arrays.asList(AUTHOR, TITLE, BOOKTITLE, YEAR)), new ArrayList<EntryType>(Arrays.asList(EDITOR, VOLUME, NUMBER, SERIES, PAGES, ADDRESS, MONTH, ORGANIZATION, PUBLISHER, NOTE, KEY)));
    }
    ...
    public List<String> validate() {
        ...
        if (!this.entries.containsKey(BOOKTITLE) || this.entries.get(BOOKTITLE).length() <= 0) {
